 ...
 * @Date 2026/4/14
 */
public class AIFlowNodeBuilder extends CallableJobFlowNodeBuilder<AIFlowNodeBuilder> {
  
    protected AIFlowNode agent;
    protected AIPlanAgent planAgent;

    public AIFlowNodeBuilder(AIFlowNode agent) {
        super(agent.getNodeId(), agent.getNodeName());
        this.agent = agent;
        this.planAgent = agent.getPlanAgent();
        
    }
    public AIFlowNodeBuilder(String nodeName) {
        super(nodeName);
    }

    public AIFlowNodeBuilder(String nodeId, String nodeName) {
