 ...
 * Host-rooted types {  LocalFilesystem} and {  LocalFilesystemWithShell} also expose
 * constructors that take the workspace root as a {@link String} path (same semantics as
 * {@link java.nio.file.Path}).
 *
 * <p>Every operation accepts a {  ChatContext} so stores can scope work to the current
 * session, user, or sandbox. Callers that are not inside a tool/agent call with a merged context
 * should pass {  ChatContext#empty()}.
 */
public interface AbstractFilesystem {
     ...
     *
     * @param runtimeContext per-call agent runtime; { ChatContext#empty()} when none
     * @param path absolute path to the directory to list (must start with '/')
     * @return LsResult with directory entries or error
     */
    LsResult ls(ChatContext runtimeContext, String path);
     ...
     */
    ReadResult read(ChatContext runtimeContext, String filePath, int offset, int limit);
     ...
     */
    WriteResult write(ChatContext runtimeContext, String filePath, String content);
     ...
     */
    GrepResult grep(ChatContext runtimeContext, String pattern, String path, String glob);
     ...
     */
    GlobResult glob(ChatContext runtimeContext, String pattern, String path);
     ...
     */
    List<FileDownloadResponse> downloadFiles(ChatContext runtimeContext, List<String> paths);
     ...
     */
    WriteResult delete(ChatContext runtimeContext, String path);
     ...
     */
    WriteResult move(ChatContext runtimeContext, String fromPath, String toPath);
     ...
     */
    boolean exists(ChatContext runtimeContext, String path);
