 ...
 */
package org.frameworkset.spi.ai.filesystem.local;
...
import org.frameworkset.spi.ai.filesystem.util.FilesystemUtils;
...
public class LocalFilesystem implements AbstractFilesystem {
    ...
    @Override
    public LsResult ls(ChatContext runtimeContext, String path) {
        Path dirPath = resolvePath(runtimeContext, path);
        if (!Files.exists(dirPath) || !Files.isDirectory(dirPath)) {
            return LsResult.success(Collections.emptyList());
        }
    ...
    @Override
    public ReadResult read(ChatContext runtimeContext, String filePath, int offset, int limit) {
        ...
        try {
            ...
            String content = FileUtil.getFileContent(resolved.toFile(), StandardCharsets.UTF_8.name());
    ...
    @Override
    public WriteResult write(ChatContext runtimeContext, String filePath, String content) {
        Path resolved = resolvePath(runtimeContext, filePath);
        ...
        try {
			...
			try (BufferedWriter writer = new BufferedWriter(
					new OutputStreamWriter(new FileOutputStream(resolved.toFile(), false), StandardCharsets.UTF_8))) {
...
//            Files.writeString(resolved, content, StandardCharsets.UTF_8);
    ...
    public EditResult edit(
        ...
        try {
            String content =
						FileUtil.getFileContent(resolved.toFile(), StandardCharsets.UTF_8.name())
                            .replace("\r\n", "\n")
                            .replace("\r", "\n");
...
//            Files.writeString(resolved, newContent, StandardCharsets.UTF_8);
    ...
    public GrepResult grep(
            ChatContext runtimeContext, String pattern, String path, String glob) {
        Path basePath;
        try {
            basePath = resolvePath(runtimeContext, path != null ? path : ".");
        } catch (SecurityException e) {
            return GrepResult.success(Collections.emptyList());
        }
    ...
    @Override
    public GlobResult glob(ChatContext runtimeContext, String pattern, String path) {
        ...
        if (!Files.exists(searchPath) || !Files.isDirectory(searchPath)) {
            return GlobResult.success(Collections.emptyList());
        }
    ...
    public List<FileUploadResponse> uploadFiles(
            ChatContext runtimeContext, List<Map.Entry<String, byte[]>> files) {
    ...
    public List<FileDownloadResponse> downloadFiles(
            ChatContext runtimeContext, List<String> paths) {
    ...
    @Override
    public WriteResult delete(ChatContext runtimeContext, String path) {
    ...
    @Override
    public WriteResult move(ChatContext runtimeContext, String fromPath, String toPath) {
    ...
    @Override
    public boolean exists(ChatContext runtimeContext, String path) {
    ...
    protected Path resolvePath(ChatContext rc, String key) {
        ...
        if (effectiveKey == null || effectiveKey.isEmpty()) {
         ...
         switch (mode) {
			 case SANDBOXED : return resolveSandboxed(effectiveKey);
			 case ROOTED : return resolveRooted(effectiveKey);
			 case UNRESTRICTED : return resolveUnrestricted(effectiveKey);
        }
		return null;
    }
    ...
    private Path resolveRooted(String effectiveKey) {
        ...
        Path target = Paths.get(effectiveKey);
    ...
    private String applyNamespacePrefix(ChatContext rc, String key) {
        if (namespaceFactory == null || key == null || key.isEmpty()) {
            return key;
        }
    ...
    private boolean hasNamespace(ChatContext rc) {
    ...
    private String resolveEntryPath(ChatContext rc, Path entry) {
    ...
    private String stripNamespacePrefix(ChatContext	 rc, Path absolutePath) {
    ...
    private List<GrepMatch> ripgrepSearch(
			ChatContext rc, String pattern, Path basePath, String includeGlob) {
    ...
    private GrepMatch parseRipgrepJsonLine(ChatContext rc, String jsonLine) {
        try {
            ...
            String filePath = resolveEntryPath(rc, Paths.get(pathText));
