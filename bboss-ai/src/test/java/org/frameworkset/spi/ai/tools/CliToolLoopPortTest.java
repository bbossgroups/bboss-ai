...
//        chatAgentMessage.setModel("qwen3.7-plus").setMaas("qwenvlplus").setRetry(3);
		...
		chatAgentMessage.setPrompt(message).setSystemPrompt("你是一个专家，可以根据用户要求生成符合要求的、完整的、可执行shell脚本，脚本中可以包含完成用户要求的多条指令代码，" +
                "并将生成的脚本交由工具执行，输出执行结果。注意事项：脚本将通过java Process调用cmd或者sh来执行，确保脚本在目标操作系统上运行。");
