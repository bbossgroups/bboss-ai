...
public class CliToolTest {
	private static org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(CliToolTest.class);
...
//        chatAgentMessage.setModel("qwen3.7-plus").setMaas("qwenvlplus").setRetry(3);
		...
		chatAgentMessage.setPrompt(question,true).setSystemPrompt("你是一个专家，可以根据用户要求生成符合要求的、完整的、可执行shell脚本，脚本中可以包含完成用户要求的多条指令代码，并将生成的脚本交由工具执行，输出执行结果");
...
//					
        ...
        aiAgent = new AIAgent();
        aiAgent.setPrompt("打印之前获取的OS版本信息");
        aiAgent.registBeanTool(new CLIShellFunctionTool(60));
        flux = aiAgent.streamChat(chatAgentMessage);
...
//					
                ...
                .doOnComplete(() -> {countDownLatch1.countDown();System.out.println();logger.info("\n=== 流完成 ===");})
                .doOnError(error ->{countDownLatch1.countDown(); logger.error("错误: " + error.getMessage(),error);})
                .subscribe();
